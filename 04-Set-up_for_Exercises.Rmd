...
You really are going to need an IDE for Python. [Spyder](https://www.spyder-ide.org/), available separately installed with PIP, or installed through Anaconda. It is a solid python focused choice. I personally use [Eclipse](https://www.eclipse.org/) as it is open sourced and you can use it for multiple languages including R (not as strong yet) and python, HTML/CSS/javascript, C/C++, Java and others. 
...
Jupyter is running first level in python. It is afterall a python module. When running the R kernel (irkernel) remmber you are running R in python. See the caution below in the contest of rMarkdown code chunks.
...
Now consider an rmarkdown session running primarily in R. The T code chunks are executed in the same R session as the rMarkdown. On the otherhand, a python code chunk is running in a python guest environment in the R level host.
