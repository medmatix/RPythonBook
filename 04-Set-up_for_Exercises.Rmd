...
All of these sites will help you approach installs and configurations in a variety of ways. As an R programmer first I installed R, then [RStudio](https://www.rstudio.com/). Once installed, RStudio is a convenient way to install the R packages as well and then Python 3 at[Python Software Foundation](https://www.python.org/). From that point on use PIP package manager for python and RGUI for package installs for python and R respectively and install jupyter see [Project Jupyter](http://jupyter.org/install). This gave the maximum flexibility for R and Python in their own environments. 
...
There are multiple kernel plugins available for jupyter notebooks. As A rule, the notebooks only use one language kernel at a time. An excepti9on is the SOS kernel.

  
#### Working in rmarkdown and Using R and Python Code Chunks

Note: I have found behavior is unpredictible to call R objects back with Python chunks run in rmarkdown (ie. inside another instance of R). The identity of R_User for rpy2 gets lost. There are limites to how deeply you should nest languages within languages.
